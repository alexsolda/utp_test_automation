...
import { rrUTP, timeout } from '../../../../helpers/globalHelper';
import LoginPage from "../../../pageobjects/LoginPage";


describe('Teste de Login (falha) - Rápido Ribeirão', () => {
  it('não deve permitir login com credenciais inválidas', async () => {
      try {

        await LoginPage.openLogin(rrUTP);
        ...
        await LoginPage.login('05440014004', 'teste1234');
        ...
        await addScreenshotToAllure(`Mensagem de erro de login - ${rrUTP}`, errorScreenshot);

    } catch (error) {
        const screenshot = await browser.takeScreenshot();
        await addScreenshotToAllure(`Falha inesperada - ${rrUTP}`, screenshot);
        throw error;  // Rejoga o erro para o teste falhar corretamente
    }
    });
});

describe('Teste de Login (sucesso) - Rápido Ribeirão', () => {
  ...
  it('deve permitir que o usuário realize o Login', async () => {
    ...
    if (isModalDisplayed) {
      ...
      if (modalText.includes('Houve uma falha na requisição. Tente novamente mais tarde.')) {
        ...
        await addScreenshotToAllure(`Falha na requisição - ${rrUTP}`, modalScreenshot);
    ...
    await addScreenshotToAllure(`Botão Entrar - ${rrUTP}`, printEnterBtn);
    ...
    await addScreenshotToAllure(`Dropdown login - ${rrUTP}`, loginForm);

    await LoginPage.login('05440014004', 'teste123');
    ...
    addAttachment(`Valor de passenger no sessionStorage - ${rrUTP}`, passengerValue, 'text/plain');
    } catch (error) {
        const modalScreenshot = await browser.takeScreenshot();
        await addScreenshotToAllure(`Falha na requisição - ${rrUTP}`, modalScreenshot);
        throw new Error(`Teste interrompido: modal com mensagem de erro detectado - "${modalText}"`);
